...
// Другой периметр и планировка: центральный горизонтальный коридор,
// по 4 комнаты сверху и снизу. Ключи комнат переиспользованы (текстуры
// пола/«голоса» комнат работают без правок). Лестница — в прачечной.
const oakgrove = {
  id: 'oakgrove',
  ...
  ground: {
    W: 48, H: 34,
    shell: { x: 9, y: 5, w: 30, h: 22 }, // cols 9..38, rows 5..26
    rooms: [
      { key: 'garage', name: 'Гараж', rect: { x: 10, y: 6, w: 7, h: 8 } },
      { key: 'utility', name: 'Прачечная', rect: { x: 18, y: 6, w: 6, h: 8 } },
      { key: 'kitchen', name: 'Кухня', rect: { x: 25, y: 6, w: 6, h: 8 } },
      { key: 'dining', name: 'Столовая', rect: { x: 32, y: 6, w: 6, h: 8 } },
      { key: 'hall', name: 'Коридор', rect: { x: 10, y: 15, w: 28, h: 3 } },
      { key: 'master', name: 'Спальня', rect: { x: 10, y: 19, w: 7, h: 7 } },
      { key: 'bath', name: 'Ванная', rect: { x: 18, y: 19, w: 6, h: 7 } },
      { key: 'living', name: 'Гостиная', rect: { x: 25, y: 19, w: 6, h: 7 } },
      { key: 'kids', name: 'Детская', rect: { x: 32, y: 19, w: 6, h: 7 } },
    ],
    doors: [
      { tx: 13, ty: 14, orient: 'h' }, // гараж→коридор
      { tx: 20, ty: 14, orient: 'h' }, // прачечная
      { tx: 27, ty: 14, orient: 'h' }, // кухня
      { tx: 34, ty: 14, orient: 'h' }, // столовая
      { tx: 13, ty: 18, orient: 'h' }, // спальня
      { tx: 20, ty: 18, orient: 'h' }, // ванная
      { tx: 29, ty: 18, orient: 'h' }, // гостиная (сдвинута от ТВ)
      { tx: 34, ty: 18, orient: 'h' }, // детская
      { tx: 9, ty: 16, orient: 'v', front: true }, // входная (запад)
    ],
    arches: [],
    windows: [
      [12, 5, 'h'], [15, 5, 'h'], [20, 5, 'h'], [27, 5, 'h'], [30, 5, 'h'], [34, 5, 'h'],
      [13, 26, 'h'], [16, 26, 'h'], [27, 26, 'h'], [31, 26, 'h'], [35, 26, 'h'],
      [38, 8, 'v'], [38, 10, 'v'], [38, 21, 'v'], [38, 23, 'v'],
      [9, 21, 'v'], [9, 23, 'v'],
    ],
    garageDoorDecor: { tx: 9, ty: 7, h: 5 },
    lamps: [
      ['garage', 13, 9.5], ['utility', 20.5, 9.5], ['kitchen', 27.5, 9.5], ['dining', 34.5, 9.5],
      ['hall', 16, 16], ['hall', 27, 16], ['hall', 34, 16],
      ['master', 13, 22], ['bath', 20.5, 22], ['living', 27.5, 22], ['kids', 34.5, 22],
    ],
    switches: [
      ['garage', 11, 13.4], ['utility', 18.4, 13.4], ['kitchen', 25.4, 13.4], ['dining', 32.4, 13.4],
      ['hall', 12, 15.4], ['master', 11, 19.4], ['bath', 18.4, 19.4],
      ['living', 25.4, 19.4], ['kids', 32.4, 19.4],
    ],
    breaker: { x: 10.5, y: 7.4 },
  },

  basement: {
    W: 30, H: 18,
    rooms: [
      { key: 'cellar', name: 'Кладовая', rect: { x: 3, y: 3, w: 10, h: 6 } },
      { key: 'workshop', name: 'Мастерская', rect: { x: 3, y: 10, w: 10, h: 5 } },
      { key: 'boiler', name: 'Котельная', rect: { x: 14, y: 3, w: 13, h: 12 } },
    ],
    doors: [
      { tx: 7, ty: 9, orient: 'h' },   // кладовая-мастерская
      { tx: 13, ty: 5, orient: 'v' },  // кладовая-котельная
      { tx: 13, ty: 12, orient: 'v' }, // мастерская-котельная
    ],
    lamps: [['cellar', 8, 6], ['workshop', 8, 12], ['boiler', 20, 8]],
    switches: [['cellar', 5, 3.6], ['workshop', 5, 10.6], ['boiler', 15.4, 3.6]],
  },

  stairs: {
    down: {
      floor: 0, tiles: { x: 20, y: 7, w: 2, h: 3 }, trigger: { x: 20, y: 7, w: 2, h: 1 },
      target: { floor: -1, x: 4, y: 6.2 }, dir: 'down',
    },
    up: {
      floor: -1, tiles: { x: 3, y: 3, w: 2, h: 3 }, trigger: { x: 3, y: 3, w: 2, h: 1 },
      target: { floor: 0, x: 20.9, y: 10.2 }, dir: 'up',
    },
  },

  van: { x: 2, y: 12, w: 3, h: 6 },
  spawn: { x: 6.5, y: 16 },
  porch: { x: 6, y: 15, w: 3, h: 3 },
  tv: { x: 26.6, y: 20, floor: 0 },

  furniture: [
    // Гараж
    ['car', 0, 10.6, 7.0, 2.5, 3.6],
    ['workbench', 0, 14.2, 6.15, 2.2, 0.85],
    ['shelfTall', 0, 10.2, 10.5, 0.75, 2.4, { tall: true }],
    ['locker', 0, 15.05, 8.3, 0.9, 1.7, { tall: true, hide: true, name: 'Шкафчик' }],
    // Прачечная (лестница в подвал — cols 20..21, rows 7..9)
    ['washer', 0, 18.15, 6.1, 1.0, 1.05],
    ['dryer', 0, 18.15, 7.3, 1.0, 1.05],
    ['waterheater', 0, 22.0, 6.2, 0.95, 1.3, { tall: true }],
    ['basket', 0, 22.1, 8.6, 0.8, 0.8, { solid: false }],
    // Кухня
    ['counter', 0, 25.1, 6.1, 2.3, 0.9],
    ['stove', 0, 27.5, 6.1, 1.2, 0.95],
    ['counterSink', 0, 28.8, 6.1, 1.2, 0.9],
    ['fridge', 0, 29.9, 6.1, 1.05, 1.15, { tall: true }],
    ['island', 0, 26.4, 9.4, 2.2, 1.25],
    // Столовая
    ['rugRound', 0, 32.6, 7.9, 3.8, 3.2, { solid: false }],
    ['diningTable', 0, 33.0, 8.4, 3.0, 1.9],
    ['chair', 0, 33.4, 7.55, 0.7, 0.7, { solid: false }],
    ['chair', 0, 34.7, 7.55, 0.7, 0.7, { solid: false }],
    ['chair', 0, 33.4, 10.45, 0.7, 0.7, { solid: false }],
    ['sideboard', 0, 32.15, 6.15, 2.0, 0.85],
    // Коридор
    ['runner', 0, 12, 15.95, 14, 1.15, { solid: false }],
    ['console', 0, 30, 15.1, 1.9, 0.7],
    ['closet', 0, 35.0, 15.08, 1.7, 0.92, { tall: true, hide: true, name: 'Чулан' }],
    ['coatrack', 0, 10.4, 15.2, 0.6, 0.6],
    // Спальня
    ['rugRect', 0, 10.6, 20.0, 4.0, 3.2, { solid: false }],
    ['bedDouble', 0, 10.4, 19.2, 2.0, 3.0],
    ['nightstand', 0, 15.2, 19.25, 0.7, 0.7],
    ['wardrobe', 0, 15.0, 21.6, 0.92, 2.3, { tall: true, hide: true, name: 'Гардероб' }],
    ['dresser', 0, 10.4, 24.0, 1.9, 0.85],
    // Ванная
    ['tub', 0, 18.15, 23.5, 2.7, 1.35],
    ['toilet', 0, 18.15, 19.3, 0.8, 1.15],
    ['sinkCab', 0, 21.4, 19.2, 1.4, 0.8],
    // Гостиная (дверь сдвинута к col 29)
    ['tvstand', 0, 25.4, 19.05, 2.5, 0.72],
    ['sofa', 0, 26.1, 23.0, 3.3, 1.15],
    ['armchair', 0, 25.0, 20.6, 1.2, 1.3],
    ['coffeeTable', 0, 26.8, 21.3, 1.9, 1.05],
    ['bookshelf', 0, 29.05, 20.0, 0.9, 2.5, { tall: true }],
    // Детская
    ['rugRound', 0, 33.0, 20.6, 2.9, 2.6, { solid: false }],
    ['bedSingle', 0, 32.15, 19.2, 1.45, 2.5],
    ['toychest', 0, 35.2, 19.25, 1.25, 0.9],
    ['desk', 0, 35.15, 21.5, 0.9, 1.9],
    ['wardrobe', 0, 32.6, 23.85, 1.7, 1.0, { tall: true, hide: true, name: 'Шкаф', rot: 0 }],
    // Подвал: кладовая (лестница вверх — cols 3..4, rows 3..5)
    ['shelfTall', -1, 6.2, 3.1, 3.0, 0.8, { tall: true }],
    ['crate', -1, 5.2, 6.5, 1.25, 1.25],
    ['barrel', -1, 11.0, 3.3, 0.95, 0.95],
    // Подвал: мастерская
    ['workbench', -1, 3.15, 10.1, 3.0, 0.9],
    ['shelfTall', -1, 9.0, 10.1, 2.0, 0.8, { tall: true }],
    ['crate', -1, 4.1, 12.6, 1.25, 1.25],
    ['locker', -1, 11.0, 10.15, 0.9, 1.7, { tall: true, hide: true, name: 'Шкафчик' }],
    // Подвал: котельная
    ['boilerTank', -1, 23.4, 3.3, 1.7, 1.9, { tall: true }],
    ['crate', -1, 15.3, 12.5, 1.45, 1.25],
    ['crate', -1, 17.0, 13.0, 1.0, 1.0],
    ['shelfTall', -1, 24.1, 7.0, 0.85, 2.6, { tall: true }],
    ['oldchair', -1, 19.2, 8.0, 0.9, 0.9],
  ],

  props: [
    ['bottle', 0, 12, 7.6], ['tool', 0, 11, 12.4],           // гараж
    ['bottle', 0, 18.4, 8.9],                                 // прачечная
    ['plate', 0, 27, 9.8], ['cup', 0, 28, 9.7],               // кухня
    ['plate', 0, 33.7, 8.9], ['bottle', 0, 34, 6.55],         // столовая
    ['book', 0, 24, 15.4],                                    // коридор
    ['book', 0, 13, 22],                                      // спальня
    ['bottle', 0, 21.9, 21],                                  // ванная
    ['book', 0, 27, 21.5], ['cup', 0, 28, 21.4],              // гостиная
    ['toy', 0, 34, 21], ['toy', 0, 35, 22.5], ['book', 0, 35.5, 20.4], // детская
    ['can', -1, 8, 6.6], ['bottle', -1, 10, 7.2],             // кладовая
    ['tool', -1, 6, 13], ['bottle', -1, 8, 12.5],             // мастерская
    ['can', -1, 20, 5], ['bottle', -1, 16, 4],                // котельная
  ],

  exterior: {
    trees: [
      [4, 4, 1.2], [7, 2.5, 1.0], [14, 2.6, 1.1], [22, 2.4, 1.2], [30, 2.6, 1.0],
      [36, 2.5, 1.2], [43, 4, 1.1], [44.5, 11, 1.0], [44, 19, 1.2], [43, 27, 1.1],
      [36, 29, 1.2], [26, 29.5, 1.0], [16, 29, 1.1], [7, 28.5, 1.2], [3, 22, 1.0],
      [2.5, 9, 1.0], [44, 29, 0.9],
    ],
    bushes: [
      [11, 4.3], [18, 4.4], [26, 4.2], [33, 4.4], [40, 7], [40, 14], [40, 22],
      [13, 27.5], [22, 27.6], [31, 27.4], [8, 22], [8, 9],
    ],
    fence: [
      [1, 1, 45.5, 0.25], [1, 31.6, 45.5, 0.25],
      [1, 1, 0.25, 30.8], [46.3, 1, 0.25, 30.8],
    ],
  },

  decor: [
    { kind: 'driveway', floor: 0, x: 2, y: 7, w: 7, h: 3.6 },
    { kind: 'vanPath', floor: 0, x: 5.5, y: 16, n: 5, step: 0.6 },
    { kind: 'painting', floor: 0, x: 16, y: 14, color: '#31424a' },
    { kind: 'painting', floor: 0, x: 31, y: 14, color: '#4a3535' },
    { kind: 'painting', floor: 0, x: 16, y: 18, color: '#3c4a35' },
    { kind: 'painting', floor: 0, x: 31, y: 18, color: '#42394f' },
    { kind: 'mat', floor: 0, x: 7.0, y: 15.5, w: 0.85, h: 0.84, color: '#4c3d26', stroke: true },
    { kind: 'bathMat', floor: 0, x: 20, y: 24 },
    { kind: 'oil', floor: 0, x: 11.6, y: 9 },
    { kind: 'cobweb', floor: 0, x: 10, y: 6, dir: 1 },
    { kind: 'cobweb', floor: 0, x: 37, y: 6, dir: -1 },
    { kind: 'cabinets', floor: 0, y: 6, blocks: [[25.1, 2.0], [28.0, 1.5]], doorX: [25.8, 26.6, 28.5, 29.2] },
    { kind: 'flowerbed', floor: 0, x: 12, w: 3.0, y: 26.15 },
    { kind: 'flowerbed', floor: 0, x: 30, w: 3.0, y: 26.15 },
    { kind: 'pipes', floor: -1, x1: 15, x2: 26, y: 3.4 },
    { kind: 'cobweb', floor: -1, x: 3, y: 3, dir: 1 },
    { kind: 'cobweb', floor: -1, x: 14, y: 3, dir: 1 },
    { kind: 'cobweb', floor: -1, x: 26, y: 3, dir: -1 },
    { kind: 'cobweb', floor: -1, x: 3, y: 10, dir: 1 },
    { kind: 'papers', floor: -1, n: 8, x: 15, y: 5, w: 10, h: 8 },
    { kind: 'pegboard', floor: -1, x: 3.4, y: 10.2 },
  ],
};

export const MAPS = [willow, oakgrove];
